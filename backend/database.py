...
async def init_db():
    """Create all tables and run safe column migrations on every startup."""
    async with engine.begin() as conn:
        await conn.run_sync(Base.metadata.create_all)

    # Safe migrations: add new columns to existing tables without failing if already present
    migrations = [
        # Advance table new columns (v25)
        "ALTER TABLE advances ADD COLUMN IF NOT EXISTS currency VARCHAR DEFAULT 'INR'",
        "ALTER TABLE advances ADD COLUMN IF NOT EXISTS exchange_rate FLOAT",
        # Invoice table new columns (v24)
        "ALTER TABLE invoices ADD COLUMN IF NOT EXISTS exchange_rate FLOAT",
        # Estimate table new columns (v24)
        "ALTER TABLE estimates ADD COLUMN IF NOT EXISTS exchange_rate FLOAT",
        # Credit note table new columns (v24)
        "ALTER TABLE credit_notes ADD COLUMN IF NOT EXISTS exchange_rate FLOAT",
        # Business table signature (v22)
        "ALTER TABLE businesses ADD COLUMN IF NOT EXISTS signature_url VARCHAR",
    ]
    async with engine.begin() as conn:
        db_url = str(engine.url)
        if 'sqlite' in db_url:
            # SQLite: no IF NOT EXISTS support — check manually
            for sql in migrations:
                table = sql.split('ALTER TABLE ')[1].split(' ADD')[0].strip()
                col = sql.split('ADD COLUMN')[1].strip().split()[0].replace('IF NOT EXISTS', '').strip()
                ...
                if col not in cols:
                    # SQLite ALTER TABLE without IF NOT EXISTS
                    sql_sqlite = sql.replace(' IF NOT EXISTS', '').split('VARCHAR')[0]
                    if 'VARCHAR' in sql:
                        sql_sqlite = sql.replace(' IF NOT EXISTS', '').replace('VARCHAR', 'TEXT')
                    elif 'FLOAT' in sql:
                        sql_sqlite = sql.replace(' IF NOT EXISTS', '')
                    try:
                        await conn.execute(text(sql_sqlite.split("DEFAULT")[0].strip()))
                    except Exception:
                        pass
        else:
