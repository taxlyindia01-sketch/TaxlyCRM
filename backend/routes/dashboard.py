...
async def get_dashboard_stats(user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    # Convert all amounts to INR using stored exchange rates
    total_collection = sum(_to_inr(i.advance_paid or 0, i.currency or "INR", i.exchange_rate) for i in invoices)
    total_revenue    = sum(_to_inr(i.subtotal or 0,     i.currency or "INR", i.exchange_rate) for i in invoices)
    total_outstanding= sum(_to_inr(i.outstanding or 0,  i.currency or "INR", i.exchange_rate) for i in invoices)
