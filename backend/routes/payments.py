...
async def delete_payment(payment_id: str, user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    """Delete a payment and reverse its effect on invoice outstanding and client balance."""
    result = await db.execute(select(Payment).where(Payment.payment_id == payment_id, Payment.tenant_id == user["tenant_id"]))
    ...
    if payment.invoice_id:
        inv_res = await db.execute(select(Invoice).where(Invoice.invoice_id == payment.invoice_id))
        invoice = inv_res.scalar_one_or_none()
        if invoice:
            new_advance_paid = max(0, (invoice.advance_paid or 0) - payment.amount)
            new_outstanding = round((invoice.total or 0) - new_advance_paid, 2)
            invoice.advance_paid = round(new_advance_paid, 2)
            invoice.outstanding = new_outstanding
            invoice.status = "paid" if new_outstanding <= 0 else ("partial" if new_advance_paid > 0 else "unpaid")

    # Reverse client outstanding balance
    if payment.client_id:
        c_res = await db.execute(select(Client).where(Client.client_id == payment.client_id))
        client = c_res.scalar_one_or_none()
        if client:
            client.outstanding_balance = (client.outstanding_balance or 0) + payment.amount
