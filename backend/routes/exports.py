...
def add_totals_row(ws, numeric_cols, start_row=2, label_col=1, label="TOTAL"):
    """Add a styled totals row at the bottom of a worksheet."""
...
async def export_invoices(start_date: Optional[str] = Query(None), end_date: Optional[str] = Query(None),
    ...
    # numeric cols for invoice summary: Subtotal(8), CGST(9), SGST(10), IGST(11), Total(12), Paid(15), Outstanding(16)
    add_totals_row(ws_sum, {8,9,10,11,12,15,16})
    # numeric for items: Qty(8), Rate(9), Amount(11)
    add_totals_row(ws_items, {8,9,11})
...
async def export_payments(start_date: Optional[str] = Query(None), end_date: Optional[str] = Query(None),
    ...
    add_totals_row(ws, {4})  # Amount col
    style_worksheet(ws)
    output = BytesIO(); wb.save(output); output.seek(0)
    return StreamingResponse(output, media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
...
async def export_clients(user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    add_totals_row(ws, {8})  # Outstanding col
    style_worksheet(ws)
    output = BytesIO(); wb.save(output); output.seek(0)
    return StreamingResponse(output, media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
...
async def export_outstanding(user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    add_totals_row(ws, {4,5,6})  # Total, Paid, Outstanding cols
    style_worksheet(ws)
    output = BytesIO(); wb.save(output); output.seek(0)
    return StreamingResponse(output, media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
