...
async def get_complete_report(start_date: str = Query(...), end_date: str = Query(...),
    ...
    # Build invoice lookup FIRST (needed for payment INR conversion)
    ...
    total_collected  = sum(_inr(p.amount or 0,
                                inv_lookup.get(p.invoice_id, {}).get('currency', 'INR'),
                                inv_lookup.get(p.invoice_id, {}).get('exchange_rate', 1)) for p in payments)
