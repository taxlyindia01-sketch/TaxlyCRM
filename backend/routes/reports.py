...
async def get_complete_report(start_date: str = Query(...), end_date: str = Query(...),
    ...
    bdata = Reference(ws_d, min_col=5, min_row=14, max_row=17)
    bcats = Reference(ws_d, min_col=4, min_row=15, max_row=17)
    ...
    for r, cn in enumerate(credit_notes, 2):
        ...
        ws_cns.cell(row=r,column=3,value=c.name if c else cn.client_name or 'Unknown')
        ws_cns.cell(row=r,column=4,value=c.gstin if c else cn.client_gstin or '')
        ...
        ws_cns.cell(row=r,column=10,value=cn.total or 0); ws_cns.cell(row=r,column=11,value=cn.status or '')
