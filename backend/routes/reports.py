...
async def get_tds_receivable_report(start_date: str = Query(...), end_date: str = Query(...),
    ...
    try:
        ...
        headers = [
            ...
            "Amount Received", "TDS Rate (%)", "TDS Amount", "Net Received After TDS"
        ]
        ...
        for p in payments:
            ...
            ws.cell(row=r, column=6, value=p.amount or 0)
            ws.cell(row=r, column=7, value="")   # TDS Rate — manual entry
            ws.cell(row=r, column=8, value="")   # TDS Amount — manual entry
            ws.cell(row=r, column=9, value="")   # Net — manual entry
            r += 1
        ...
        for adv in advances:
            ...
            ws.cell(row=r, column=4, value=c.name if c else "Unknown")
            ws.cell(row=r, column=5, value=c.gstin if c else "")
            ws.cell(row=r, column=6, value=adv.amount or 0)
