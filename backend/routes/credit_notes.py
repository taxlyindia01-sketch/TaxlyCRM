...
def _calc_taxes(items, is_same_state):
    cgst = sgst = igst = 0.0
    for item in items:
...
async def create_credit_note(cn_data: CreditNoteCreate, user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    cgst, sgst, igst = _calc_taxes(cn_data.items, business.state == client.state)
    ...
    invoice.outstanding = round((invoice.outstanding or 0) - total, 2)
...
async def download_credit_note_pdf(credit_note_id: str, user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    if biz.get('authorised_signatory'):
        elements.append(Spacer(1, 0.3*inch))
        from_s = ParagraphStyle('FromStyle', parent=styles['Normal'], fontSize=9, leading=12)
        sig_table = Table([['', Paragraph(f"<b>Authorised Signatory</b><br/>{biz.get('authorised_signatory')}", from_s)]], colWidths=[4.5*inch, 2*inch])
        sig_table.setStyle(TableStyle([('ALIGN',(1,0),(1,0),'CENTER'), ('VALIGN',(1,0),(1,0),'TOP'), ('LINEABOVE',(1,0),(1,0),1,colors.black)]))
