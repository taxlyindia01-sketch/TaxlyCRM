...
async def download_estimate_pdf(estimate_id: str, user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    if biz.get('authorised_signatory') or biz.get('signature_url'):
        ...
        sig_table = Table([[sig_img_cell, sig_label]], colWidths=[1.5*inch, 5*inch])
        sig_table.setStyle(TableStyle([('ALIGN',(0,0),(0,0),'CENTER'), ('ALIGN',(1,0),(1,0),'CENTER'),
                                        ('VALIGN',(0,0),(-1,-1),'BOTTOM'), ('LINEABOVE',(1,0),(1,0),1,colors.black)]))
