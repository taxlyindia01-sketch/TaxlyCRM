...
async def create_invoice(invoice_data: InvoiceCreate, user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    # Business profile is optional for invoice creation
    # If not set up, default to same-state (CGST/SGST) to avoid blocking workflow
    if not client:
        raise HTTPException(status_code=400, detail="Client not found. Please add the client first.")
    if not business:
        import types
        business = types.SimpleNamespace(state=client.state or "")  # default: same state as client
...
async def cancel_invoice(invoice_id: str, user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    # Zero out all financial fields so the invoice is excluded from dashboard revenue
    invoice.status = "cancelled"
    invoice.outstanding = 0
    invoice.advance_paid = 0
    # Keep subtotal/total/cgst/sgst/igst as-is for audit trail (they show original values)
    # Dashboard already filters by status or sums all — we handle this by storing a
    # cancelled flag. The dashboard sums ALL invoices, so we set total = 0 for cancelled ones.
    invoice.total = 0
    invoice.subtotal = 0
    invoice.cgst = 0
    invoice.sgst = 0
...
async def download_invoice_pdf(invoice_id: str, user: dict = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    if not client:
        raise HTTPException(status_code=400, detail="Client not found")
    if not business:
        # Business profile not set up yet — use placeholder dict so PDF still generates
        biz = {
            "business_name": "Your Business", "gstin": "", "address": "",
            "city": "", "state": "", "pincode": "", "phone": "", "email": "",
            "logo_url": None, "qr_code_url": None, "signature_url": None,
            "authorised_signatory": "", "upi_id": "", "bank_name": "",
            "account_number": "", "ifsc_code": "", "swift_code": "",
            "terms_and_conditions": "Please complete Business Settings."
        }
    else:
        biz = None  # will be set below via serialize_doc

    inv = serialize_doc(invoice)
    biz = biz if biz is not None else serialize_doc(business)
